...
class ContactItem(Frame):
    ...
    def __init__(self, parent, onclick):
        ...
        Frame.config(self, borderwidth=2, relief=GROOVE)

        # == Line 1
        self.title_frame = Frame(self)
        self.last_message_time_frame = Frame(self)
        self.friend_ip_frame = Frame(self)
        self.message_frame = Frame(self)

        # last_message
        self.message_frame.pack(side=BOTTOM, fill=BOTH, expand=True, pady=(0, 5), padx=3)
        self.last_message = Label(self.message_frame, text="recent message", font=('新宋体', 13), fg='black')
        ...
        self.title = Label(self.title_frame, text="Title", font=("微软雅黑", 15, 'bold'))
        self.title.pack(side=LEFT, fill=None, anchor=W)

        # 最后一条消息的时间
        self.last_message_time_frame.pack(side=TOP, expand=FALSE, fill=BOTH)
        self.last_message_time = Label(self.last_message_time_frame, text="date", font=('微软雅黑', 10), fg='#575757')
        self.last_message_time.pack(side=RIGHT, anchor=E)

        # 好友的ip
        self.friend_ip_frame.pack(side=TOP, expand=FALSE, fill=BOTH)
        self.friend_ip = Label(self.friend_ip_frame, text="ip", font=('微软雅黑', 10), fg='#575757')
